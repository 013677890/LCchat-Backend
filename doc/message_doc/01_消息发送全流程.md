...
Kafka "msg.push" 消费流程
  ...
  ├─ ① 目标投递（按 conv_type 分支）：
  │    conv_type=1 (单聊)：
  │      SendToUser(receiver_uuid, frameBytes)
  │
  │    conv_type=2 (群聊)：
  ...
  │
  └─ ② 发送方多端同步（Self-Sync，原理见下方说明）：
       from_uuid 非空 → SendToUserExcludeDevice(from_uuid, device_id, frameBytes)
       （向发送方除当前设备外的所有在线设备推送）
```

> **多端同步（Self-Sync）说明**：  
> 发送方在手机上发了一条消息，同时登录在电脑端的 App 也应立即看到这条消息。  
> Connect 消费到 MsgPushEvent 后，除了向目标（对端/群成员）投递，还会向 `from_uuid` 下除 `device_id` 以外的其他在线设备服发一份。  
