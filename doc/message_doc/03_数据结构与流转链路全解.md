...
[3. Msg-Service (逻辑与持久化)]
    ...
    ↓ (Protobuf - MsgPushEvent 写入Kafka)
[4. Kafka Message Queue]
    ↓ (Protobuf - MsgPushEvent 消费)
[5. Connect-Service (接入与下发层)]
    ↓ (Protobuf - MessageEnvelope -> MsgItem 封包)
[6. 接收端] (WebSocket Binary - 最终业务展示)
```
...
消息落库后，向外分发。Msg-Service 不直接调取 Connect 的 RPC，而是丢入一条分发事件，解耦上下游。

**Kafka 分区策略**：
- **Partition Key**：`conv_id`（不是 receiver_uuid）
- **原因**：同一会话的连续消息必须落入同一个 Partition，确保 Connect 串行消费时严格有序
...
message MsgPushEvent {
  ...
  ConvType conv_type = 4;   // Connect 据此决定扩散策略（1=单聊写扩散 2=群聊读扩散）
...
- `device_id`：填发送方当前设备 ID（Connect 据此排除该设备，避免发送方收到重复消息）
- `data`：将落库后的 `Message` 模型字段转化为 `MsgItem` proto 并序列化为 bytes

---

### 阶段五：Kafka -> Connect-Service -> Client

Connect-Service 作为消费者消费 Kafka，解码出 `MsgPushEvent`。Connect 的投递分两步：

**① 目标投递**（按 conv_type 分支）：
- 单聊(conv_type=1)：直接 `SendToUser(receiver_uuid)`
- 群聊(conv_type=2)：查询 Redis 群成员列表，过滤 `from_uuid`，遍历在线成员投递

**② 发送方多端同步（Self-Sync）**：
- 向 `from_uuid` 下除 `device_id` 以外的所有在线设备投递一份
- 场景：发送方在手机上发消息，同时登录在电脑端的客户端也应立即看到这条消息
- 客户端通过 `MsgItem.from_uuid == 自己的UUID` 判断这是多端同步而不是对方发的消息

投递通过 WebSocket 下发二进制，协议为 **MessageEnvelope**。
